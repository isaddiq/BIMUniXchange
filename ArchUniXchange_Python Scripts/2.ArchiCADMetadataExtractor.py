        ...
        try:
            ...
            with open(csv_path, 'w', newline='', encoding='utf-8-sig') as csvfile:
                ...
                for elem_data in elements:
                    ...
                    # Try to extract IFC type from properties
                    ifc_type = ""
                    for key in ['IFC_EntityType', 'IfcType', 'IFC Type', 'IFC_Type']:
                        if key in row_data:
                            ifc_type = row_data[key]
                            break
                    
                    # Fallback to element type mapping if no IFC type found
                    if not ifc_type:
                        ifc_type_map = {
                            'Wall': 'IfcWall',
                            'Slab': 'IfcSlab',
                        ...
                        ifc_type = ifc_type_map.get(elem_type, 'IfcBuildingElementProxy')
                    
                    self.summary.record_element(elem_type, param_count, ifc_type)
